...
struct StrokesModel {
    ...
    static func calculateStrokePlayStrokeHoles(roundViewModel: RoundViewModel, holes: [Hole]) {
        for golfer in roundViewModel.golfers {
            guard let courseHandicap = roundViewModel.courseHandicaps[golfer.id] else {
                print("Warning: No course handicap found for golfer \(golfer.firstName) \(golfer.lastName)")
                continue
            }
    ...
    static func calculateGameStrokeHoles(roundViewModel: RoundViewModel, golfers: [Golfer]) {
        ...
        let golfersWithHandicaps = golfers.map { golfer -> (Golfer, Int) in
            ...
            print("Debug StrokesModel: \(golfer.firstName) \(golfer.lastName) - Playing Handicap: \(handicap)")
        ...
        print("Debug StrokesModel: Lowest Handicap Player: \(lowestHandicapPlayer.firstName) \(lowestHandicapPlayer.lastName) with handicap \(lowestHandicapValue)")
    ...
    static func calculateMatchPlayStrokeHoles(roundViewModel: RoundViewModel, golfer: Golfer, gameHandicap: Int, isLowestHandicap: Bool) {
        print("Debug StrokesModel: Match Play Handicap for \(golfer.firstName) \(golfer.lastName): \(gameHandicap)")
        ...
        print("Debug StrokesModel: Match Play Stroke Holes - \(golfer.firstName) \(golfer.lastName): \(roundViewModel.matchPlayStrokeHoles[golfer.id] ?? [])")
    }

    static func calculateBetterBallStrokeHoles(roundViewModel: RoundViewModel, golfer: Golfer, gameHandicap: Int) {
        print("Debug StrokesModel: Better Ball Handicap for \(golfer.firstName) \(golfer.lastName): \(gameHandicap)")
        ...
        if let strokeHoles = roundViewModel.strokeHoles[golfer.id] {
            ...
            print("Warning: No stroke holes found for \(golfer.firstName) \(golfer.lastName)")
            roundViewModel.betterBallStrokeHoles[golfer.id] = []
        }
        
        print("Debug StrokesModel: Better Ball Stroke Holes - \(golfer.firstName) \(golfer.lastName): \(roundViewModel.betterBallStrokeHoles[golfer.id] ?? [])")
    }
    ...
    static func calculateNinePointStrokeHoles(roundViewModel: RoundViewModel) {
        ...
        for player in golfers {
            ...
            print("Debug: Nine Point Stroke Holes for \(player.firstName) \(player.lastName): \(roundViewModel.ninePointStrokeHoles[player.id] ?? [])")
        }
    }
}
