...
struct BetterBallPressModel {
    ...
    roundViewModel.betterBallPresses.append((startHole: atHole, matchStatusArray: Array(repeating: 0, count: 18), winner: nil, winningScore: nil, winningHole: nil))
    ...
    if let lastPress = roundViewModel.betterBallPresses.last {
        ...
        let matchScore = roundViewModel.betterBallMatchArray[..<roundViewModel.currentHole].reduce(0, +)
        if matchScore > 0 {
    ...
    static func updateBetterBallPressMatchStatus(roundViewModel: RoundViewModel, pressIndex: Int, for currentHoleNumber: Int) {
        ...
        if abs(pressStatus) > remainingHoles {
            ...
            roundViewModel.betterBallPresses[pressIndex].winningHole = 18
        }
    }
