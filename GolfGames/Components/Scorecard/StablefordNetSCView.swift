//
//  StablefordNetSCView.swift
//  GolfGames
//
//  Created by Mike Dietrich on 9/16/24.
//

import SwiftUI

struct StablefordNetSCView: View {
    ...
    private func playerRow(for golfer: Golfer, holes: ClosedRange<Int>, showTotal: Bool, showFirstColumn: Bool) -> some View {
        HStack(spacing: 0) {
            ...
            ForEach(holes, id: \.self) { hole in
                if let points = roundViewModel.stablefordNetScores[hole]?[golfer.id] {
                    Text("\(points)")
                        .frame(width: scoreCellWidth, height: scoreCellHeight)
            ...
            let totalPoints = holes.reduce(0) { total, hole in
                total + (roundViewModel.stablefordNetScores[hole]?[golfer.id] ?? 0)
            }
            Text("\(totalPoints)")
                .frame(width: scoreCellWidth, height: scoreCellHeight)
                .background(Color(UIColor.systemGray4))
                .fontWeight(.bold)
            if showTotal {
                let grandTotalPoints = roundViewModel.stablefordNetTotalScores[golfer.id] ?? 0
...
struct StablefordNetSCView_Previews: PreviewProvider {
