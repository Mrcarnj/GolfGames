...
export default function RoundSummary() {
  ...
  if (!round) {
    return <View style={styles.container}><Text>Round not found.</Text></View>;
  }
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {round.golfers.map((g, idx) => (
        <View key={g.id} style={styles.golferRow}>
          <Text style={styles.golferName}>{g.firstName} {g.lastName}</Text>
          <Text>Scores: {Object.values(round.scores[g.id] || {}).join(', ')}</Text>
        </View>
      ))}
