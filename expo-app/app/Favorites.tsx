      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.favoriteRow}>
            <TouchableOpacity style={styles.favoriteInfo} onPress={() => router.push({ pathname: '/CourseDetails', params: { courseId: item.courseId } })}>
              <Text style={styles.courseName}>{item.courseName}</Text>
              <Text style={styles.teeName}>{item.teeName}</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.removeBtn} onPress={() => removeFavorite(item.courseId, item.teeId)}>
              <Text style={{ fontSize: 20, color: '#bbb' }}>✕</Text>
            </TouchableOpacity>
          </View>
        )}
        ListEmptyComponent={<Text style={{ marginTop: 32 }}>No favorites yet.</Text>}
      />
