...
export default function CourseDetails() {
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      ...
      const teesSnap = await getDocs(
        collection(db, "courses", courseId, "tees"),
      );
  ...
  const isFavorited =
    selectedTee &&
    favorites.some(
      (f) => f.courseId === courseId && f.teeId === selectedTee.id,
    );
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <Text>
        Rating: {course.rating || "N/A"} | Slope: {course.slope || "N/A"} | Par:{" "}
        {course.par || "N/A"}
      </Text>
      {/* Tees */}
      ...
      <ScrollView horizontal style={styles.teeRow}>
        {tees.map((tee) => (
          <TouchableOpacity
            ...
            style={[
              styles.teeChip,
              selectedTee?.id === tee.id && styles.selectedTee,
            ]}
            onPress={() => setSelectedTee(tee)}
          >
            <Text>
              {tee.tee_name} ({tee.yardage} yds)
            </Text>
          </TouchableOpacity>
        ))}
        ...
        {selectedTee && (
          ...
          >
            <Text
              style={{ fontSize: 24, color: isFavorited ? "#FFD700" : "#bbb" }}
            >
              {isFavorited ? "★" : "☆"}
            </Text>
          </TouchableOpacity>
