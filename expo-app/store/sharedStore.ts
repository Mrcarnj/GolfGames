...
export interface SharedState {
  ...
  addFavorite: (courseId: string, courseName: string, teeId: string, teeName: string) => void;
...
export const useSharedStore = create<SharedState>()(
  persist(
    (set) => ({
      ...
      addFavorite: (courseId, courseName, teeId, teeName) => set((state) => {
        const exists = state.favorites.some(f => f.courseId === courseId && f.teeId === teeId);
        if (exists) return {};
        return { favorites: [{ courseId, courseName, teeId, teeName }, ...state.favorites] };
      }),
      removeFavorite: (courseId, teeId) => set((state) => ({
        favorites: state.favorites.filter(f => !(f.courseId === courseId && f.teeId === teeId)),
      })),
    }),
    {
      name: 'shared-store',
      storage: zustandAsyncStorage,
      partialize: (state) => ({ golfers: state.golfers, selectedTees: state.selectedTees, courseHandicaps: state.courseHandicaps, selectedCourse: state.selectedCourse, roundId: state.roundId, recentCourseSearches: state.recentCourseSearches, favorites: state.favorites }),
    }
  )
); 